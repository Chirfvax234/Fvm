...
function QBCore.Functions.Notify(text, textype, length)
	...
	if textype == nil or textype == "" or texttype == "info" then 
		textype = "primary"
	end
    ...
    exports["ra1derBase"]:SendAlert(text,textype,length)
end
...
function QBCore.Functions.IsSpawnPointClear(coords, radius)
...
function QBCore.Functions.Progressbar(name, label, duration, useWhileDead, canCancel, disableControls, animation, prop, propTwo, onFinish, onCancel, icon)
    -- if GetResourceState('tgiann-progressbar') ~= 'started' then error('progressbar needs to be started in order for QBCore.Functions.Progressbar to work') end
...
function QBCore.Functions.GetVehicleProperties(vehicle)
    if DoesEntityExist(vehicle) then
        ...
        return {
            ...
            CamFilmi = GetVehicleWindowTint(vehicle),
...
function QBCore.Functions.SetVehicleProperties(vehicle, props)
    if DoesEntityExist(vehicle) then
        ...
        if props.CamFilmi then
            SetVehicleWindowTint(vehicle, props.CamFilmi)
        end
