...
window.addEventListener('message', (event) => {
    if (event.data.type === 'open') {
        ...
        $(".test-menu-money").html("25$ (50 Saniye)")
        testCarTime = 
...
function menuOpen(newCarData) {
    ...
    Object.keys(carData).forEach(function(element) {
        ...
        for (let index = 0; index < carData[element].length; index++) {
            ...
            let porno = (xelement.carName).toUpperCase()
            if (xelement.count > 0) {
...
function setCarInfo(data) {
    ...
    $.post('https://tgiann-vehicleshop/getCarDataAndSpawnCar', JSON.stringify({model: data.carModel}), function(cbData) {
        $("#carName").html((data.carName).toUpperCase())
        $("#carStock").html('Stok: '+data.count)
        if (inSuleyman) {
            $("#npcPriceMenu2").css("display", "block")
            $("#dealerPriceMenu").css("display", "none")
            $("#vehPrice").html(setCurrency(data.price))
        } else {
            $("#npcPriceMenu2").css("display", "none")
            $("#dealerPriceMenu").css("display", "block")
            dealerBuyPercentage = cbData.dealerBuyPercentage
            $("#vehSellPrice").html(setCurrency(Math.round((data.price*dealerBuyPercentage)/100)))
            $("#vehBuyPrice").html(setCurrency(data.price))
        }
    ...
    if (data.count > 0) {
        $(".car-buy-button").html("Satın Al")
    } else {
        $(".car-buy-button").html("Stokta Kalmadı")
    }
}
...
window.addEventListener('message', function(event) {
    if (event.data.action === 'closeMenu') {
        // JavaScript tarafında closeMenu fonksiyonunu çağırma
        closeMenu(event.data.buy, event.data.test);
    }
});
...
$(document).on("click",".car-buy-button", function() {
    if (inSuleyman) {
        ...
        $(".buy-menu-money").html(setCurrency(Math.round((selectedCar.price*dealerBuyPercentage)/100))) 
    }
